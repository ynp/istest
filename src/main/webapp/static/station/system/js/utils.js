 ...
 * initDateTime('d',1,'yyyy-MM-dd');
 */
function initDateTimeToday(type, step, format) {
	_utilsDateStep = step;
	var start = Date.prototype.formatdate(new Date(), format);
	var end = Date.prototype.formatdate(DateAdd(type, step, new Date()), format);
	$("#starttime").val(start);
	$("#endtime").val(end);
}
/**
 ...
 * @param {Object} step	开始时间距离结束时间的间隔。
 * @param {Object} format 格式化样式
 * @param {Object} startTaget 设置开始时间的标签ID或class
 * @param {Object} endTaget 设置结束时间的标签ID或class
 * 例如：开始时间为当前时间前一天 
 * initDateTime('d',-1,'yyyy-MM-dd','#starttime','#endtime');
 * initDateTime('d',-1,'yyyy-MM-dd','.starttime','.endtime');
 */
function initDateTimeByTaget(type, step, format, startTaget, endTaget) {
	_utilsDateStep = step;
	var start = Date.prototype.formatdate(DateAdd(type, step, new Date()), format);
	var end = Date.prototype.formatdate(new Date(), format);
	$(startTaget).val(start);
	$(endTaget).val(end);
}

function initOneDateTimeByTaget(format, taget) {
	var start = Date.prototype.formatdate(new Date(), format);
	$(taget).val(start);
}
...
function setDatetimepicker(type) {
	...
	if(type === "d") { // 日
		...
		option.endDate = DateAdd('d', 1, new Date());
		option.startDate = DateAdd(dateType, 1, new Date());
...
function isDifferNDay(startTime,endTime,n) {
	...
	if(startTime.length>0 && endTime.length>0){  
		if(datadiff<=0||datadiff>time){  
			swal("提示","开始时间应小于结束时间并且间隔小于3天，请检查!");
			return false;  
		}
	}else{
		swal("提示","开始时间、结束时间不能为空，请检查!");
		return false;  
	}
	return true;  
}
...
/**
 * 获取所选时间N天 后的时间
 * @param val 日期(格式2017-06-16)
 * @param n 间隔天数
 * @return 2017-06-16
 */
function getStringDate(val,n){
	var selectDate = new Date(val.replace("-","/").replace("-","/"));
	var date = DateAdd( "d", n, selectDate);
	return Date.prototype.formatdate(date,"yyyy-MM-dd");
}
 ...
 * @param val 日期(格式2017-06-16)
 * @param n 间隔分钟数
 * @return 2017-06-16 00:15
 */
function getStringinuteDate(val,n){
	var selectDate = new Date(val.replace("-","/").replace("-","/"));
	var date = DateAdd( "minute", n, selectDate);
	return Date.prototype.formatdate(date,"yyyy-MM-dd hh:mm");
}

/**
 * 获取ismodify代表意思 
 * @param val 无修正-0，模型重新计算-1, 人工修正 -2, 补录 -3
 * @returns {String}
 */
